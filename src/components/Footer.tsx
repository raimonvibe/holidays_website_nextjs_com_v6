...
const Footer: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const handleSnowflakes = (element: HTMLElement) => {
    ...
    if (mybutton) {
      ...
      mybutton.addEventListener('mouseenter', () => handleSnowflakes(mybutton));
    }

    const addSnowflakeListeners = () => {
      const socialButtons = document.querySelectorAll('.Social:not(.footer-link)');
      socialButtons.forEach(button => {
        button.addEventListener('mouseenter', () => handleSnowflakes(button as HTMLElement));
      });
    };
    ...
    return () => {
      ...
      if (mybutton) {
        ...
        mybutton.removeEventListener('mouseenter', () => handleSnowflakes(mybutton));
      }
      const socialButtons = document.querySelectorAll('.Social:not(.footer-link)');
      socialButtons.forEach(button => {
        button.removeEventListener('mouseenter', () => handleSnowflakes(button as HTMLElement));
      });
  ...
  const socialLinks = [
    ...
    { href: "https://www.facebook.com/profile.php?id=61563450007849", icon: faFacebook, label: "Link to Facebook" },
  ];
    ...
    <footer className="container-fluid">
      {socialLinks.map((link, index) => (
        <React.Fragment key={link.href}>
          ...
          <Link
            href={link.href}
            className={`footer-link footer-social ${link.className || ''}`}
            target="_blank"
            rel="noreferrer"
